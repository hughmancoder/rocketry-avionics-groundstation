...
bytes that you want to send uint8_t* output		--> pointer to the
beginning of output array uint16_t w_length	--> detemines number of bytes
...
*\ name        : LoRa_setLowDaraRateOptimization
                ...
                description : set the LowDataRateOptimization flag. Is is
mandated for when the symbol length exceeds 16ms.

                arguments   :
                        LoRa*	LoRa         --> LoRa object handler
                        uint8_t	value        --> 0 to disable, otherwise to
enable
...
*\ name        : LoRa_setAutoLDO
                ...
                description : set the LowDataRateOptimization flag automatically
based on the symbol length.
...
void LoRa_setAutoLDO(LoRa *_LoRa) {
  ...
  LoRa_setLowDaraRateOptimization(
      _LoRa, (long)((1 << _LoRa->spredingFactor) /
